...
#     embed = discord.Embed(
#         title=f"📋 Review Item #{index + 1}",
#         color=color
#     )
    
#     # Message text
#     text = item['text'][:500]  # Truncate if too long
...
#     embed.add_field(
#         name="Model Says",
#         value=f"{model_label} ({confidence}: {score:.1%})",
#         inline=True
#     )
    
#     # User claim
#     user_label = "🔴 Toxic" if item['user_claimed_label'] == 1 else "✅ Safe"
#     embed.add_field(
#         name="User Says",
#         value=user_label,
#         inline=True
...
#     # Dispute info
#     hours_ago = item.get('hours_pending', 0)
#     if hours_ago < 1:
...
#     embed.add_field(
#         name="User History",
#         value=f"{violations} previous violations",
#         inline=True
#     )
    
#     # IDs for admin reference
#     embed.set_footer(
#         text=f"Feedback ID: {item['feedback_id']} | Log ID: {item['log_id']} | User: {item['user_id']}"
#     )
...
class ReviewItemView(discord.ui.View):
    ...
    # @discord.ui.button(label="✅ Model Correct", style=discord.ButtonStyle.success, custom_id="agree_model")
    # async def agree_with_model(self, interaction: discord.Interaction, button: discord.ui.Button):
    #     """Admin agrees with model's decision"""
    #     if str(interaction.user.id) != self.admin_id:
    #         await interaction.response.send_message("❌ Only the reviewing admin can use these buttons", ephemeral=True)
    #         return
    ...
    #     if self.item.get('type') == 'uncertain':
    #     # Create a "proactive" feedback entry so we can update it
    #         feedback_id = await self.db.record_user_dispute(
    #             log_id=self.item['log_id'],
    #             user_id=self.item['user_id'],
    #             server_id=str(interaction.guild.id),
    #             platform='discord',
    #             user_claimed_label=0, # Default placeholder
    #             dispute_reason="Admin Proactive Review"
    #         )
    #         self.item['feedback_id'] = feedback_id # Update item with new ID
    #     # Record decision
    #     await self.db.admin_review_feedback(
    #         feedback_id=self.item['feedback_id'],
    #         admin_id=self.admin_id,
    #         decision='agree_with_model',
    #         notes=f"Reviewed via dashboard"
    #     )
        
    #     # Update message
    #     embed = interaction.message.embeds[0]
    #     embed.color = discord.Color.green()
    #     embed.title = "✅ APPROVED - Model was correct"
    ...
    # @discord.ui.button(label="❌ User Correct", style=discord.ButtonStyle.danger, custom_id="agree_user")
    # async def agree_with_user(self, interaction: discord.Interaction, button: discord.ui.Button):
    #     """Admin agrees with user's dispute"""
    #     if str(interaction.user.id) != self.admin_id:
    #         await interaction.response.send_message("❌ Only the reviewing admin can use these buttons", ephemeral=True)
    #         return
    ...
    #         feedback_id=self.item['feedback_id'],
    #         admin_id=self.admin_id,
    #         decision='agree_with_user',
    #         notes=f"False positive - user was correct"
    #     )
        
    #     # Update message
    #     embed = interaction.message.embeds[0]
    #     embed.color = discord.Color.blue()
    #     embed.title = "🔄 OVERRIDDEN - User was correct (false positive)"
